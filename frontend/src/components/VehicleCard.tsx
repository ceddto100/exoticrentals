...
export const VehicleCard: React.FC<VehicleCardProps> = ({ car }) => {
  ...
  return (
    <Link to={`/vehicle/${car.id}`} className="group block" aria-label={`${make} ${model}`}>
      <div className="glass-card rounded-2xl overflow-hidden transition-all duration-500 hover:border-cyan-500/30 h-full flex flex-col neon-border">
        {/* Image Container */}
        <div className="relative h-56 overflow-hidden">
          ...
          {/* Category Badge */}
          <div className="absolute top-4 right-4 px-3 py-1.5 rounded-full glass-card text-xs font-bold text-cyan-400 border border-cyan-500/30">
            {category}
          </div>
          ...
          {!isAvailable && (
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center">
              <div className="px-6 py-3 rounded-xl bg-pink-500/20 border border-pink-500/50 text-pink-400 font-bold uppercase tracking-wider text-sm">
                Currently Booked
              </div>
            </div>
          )}
        ...
        <div className="p-6 flex-grow flex flex-col">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg glass-card text-sm text-gray-300">
              {fuelType === 'Electric' ? (
                ...
                <Fuel className="h-4 w-4 text-pink-500" />
              )}
              <span>{fuelType}</span>
            </div>
          </div>
          ...
          {/* Price and CTA */}
          <div className="mt-auto pt-4 border-t border-cyan-500/10 flex items-center justify-between">
            <div>
              <span className="text-2xl font-black gradient-text">${price}</span>
              <span className="text-gray-500 text-sm ml-1">/ day</span>
            </div>
            <div className="flex items-center gap-2 text-cyan-400 font-semibold text-sm group-hover:text-cyan-300 transition-colors">
