            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              ...
              {user ? (
                <>
                  ...
                  <button onClick={handleLogout} className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-red-400 hover:bg-gray-800">Sign Out</button>
                </>
              ) : (
                <Link to="/login" className="block px-3 py-2 rounded-md text-base font-medium text-amber-300 hover:bg-gray-800" onClick={() => setIsMobileMenuOpen(false)}>Log In</Link>
              )}
            </div>
          </div>
        )}
